...
double precision,dimension(:),allocatable::nada, dato
...
if ( columna > 1 ) then
   ...
   do i=1,num_data
      ...
      if ( dato(i) > maxim ) maxim=dato(i)
      if ( dato(i) < minim ) minim=dato(i)
      
      cumul=cumul+dato(i)
      cumul2=cumul2+dato(i)**2
...
DO k=1,num_data
   DO j=1,num_div2
      ...
      IF (dato(k) < (maxim+delta_2-(j-1)*delta)) THEN
